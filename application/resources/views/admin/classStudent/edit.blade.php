...
@section('content')
    <div class="row">
        <div class="col-xs-12">
            <div class="box">
                <form class="form-horizontal" method="post" action="{{ route('admin.classStudent.update', $classStudent['id']) }}">
                    <div class="box-body">
                                ...
                                <select class="form-control select2" id="class_schedule_id" name="class_schedule_id"
                                    ...
                                    @foreach($classSchedules as $classSchedule)
                                        <option value="{{ $classSchedule['id'] }}"
                                                @if($classSchedule['id'] == old('class_schedule_id', $classStudent['class_schedule_id'])) selected @endif>{{ $classSchedule['class_info']}}</option>
                                    @endforeach
                                </select>
                                ...
                                <select class="form-control select2" id="student_id" name="student_id"
                                    ...
                                    @foreach($students as $student)
                                        <option value="{{ $student['id'] }}"
                                                @if($student['id'] == old('student_id', $classStudent['student_id'])) selected @endif>{{ $student['name'] }}
                                        </option>
                                    @endforeach
                                </select>
                        ...
                        <div class="form-group @if($errors->first('teacher_name')) has-error @endif">
                            ...
                            <div class="col-sm-10">
                                <input type="text" class="form-control" id="teacher_name" name="teacher_name"
                                        placeholder="Teacher" value="{{ old('teacher_name' , $classStudent['teacher_name']) }}">
