...
@section('content')
    <div class="row">
        <div class="col-xs-12">
            <div class="box">
                <form class="form-horizontal" method="post" action="{{ route('admin.classStudent.update', $classStudent) }}">
                    <div class="box-body">
                                ...
                                <select class="form-control select2" id="class_schedule_id" name="class_schedule_id"
                                    ...
                                    @foreach($classSchedules as $classSchedule)
                                        <option value="{{ $classSchedule->id }}"
                                                @if($classSchedule->id == old('class_schedule_id', $classStudent->class_schedule_id)) selected @endif>{{ $classSchedule->getClassInfo() }}</option>
                                    @endforeach
                                </select>
